...
bool RtlUsbAdapter::send_packet(uint8_t *packet, size_t length) {
  ...
  if (rc == LIBUSB_SUCCESS && actual_length == length) {
    // _logger->debug("Packet sent successfully, length: {},used time {}ms", length, elapsed.count());
    return true;
  } else {
    _logger->error("Failed to send packet, error code: {}, actual_length: {}", rc, actual_length);
    return false;
  }
}
